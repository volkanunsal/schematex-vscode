...
- **Spec coverage:** fence detection + live update (Task 3, riding VS Code's built-in preview refresh per the spec's data-flow section) — covered; per-diagram config (Task 2) — covered; render + error card (Task 4) — covered; PNG/PDF export (Task 4 + 5) — covered; AGPL-3.0 licensing (Task 1) — covered; testing gap for actual SVG rendering — called out explicitly in Task 5 rather than hidden.
